...
import { musicTaste } from "../recommendations";
...
const videoSchema = z.object({
  ...
  snippet: z.object({ title: z.string().max(500), channelTitle: z.string().max(200), channelId: z.string().max(128),
    ...
    tags: z.array(z.string()).optional(), defaultAudioLanguage: z.string().optional(),
...
export function normalizeYouTubeVideo(value: unknown, region?: string): MusicTrack | null {
  ...
  const taste = musicTaste(video.snippet.tags ?? [], video.topicDetails?.topicCategories ?? [], video.snippet.defaultAudioLanguage);
  return { provider: "youtube", id: video.id, title: video.snippet.title, artist: video.snippet.channelTitle,
    channelId: video.snippet.channelId, duration, genre: taste.genres.join(", ") || "Music", taste, artwork, artworkMirrors: [],
