...
export function MediasoupDiagnostic() {
  ...
  async function run() {
    ...
    const credential = await response.json() as { credential?: string; mediaServerUrl?: string; roomId?: string; error?: string };
    if (!response.ok || !credential.credential || !credential.mediaServerUrl || !credential.roomId) throw new Error(credential.error ?? "A voice channel ID is required.");
    client.current = new MediaClient(); client.current.subscribe(() => setState(client.current?.snapshot().state ?? "Disconnected")); await client.current.connect({ credential: credential.credential, mediaServerUrl: credential.mediaServerUrl, roomId: credential.roomId }); await client.current.start("mic", true);
  }
  return <main className="p-10 text-white"><h1 className="text-xl font-bold">VAL mediasoup diagnostic</h1><p className="mt-2 text-sm text-slate-400">Development only. Add <code>?channelId=...</code> for a voice channel you can access.</p><p className="mt-4">State: {state}</p><button className="app-button-primary mt-4 h-10 rounded-lg px-4" onClick={() => void run().catch((error: unknown) => setState(error instanceof Error ? error.message : "Failed"))} type="button">Join and publish microphone</button><button className="app-button-secondary ml-2 h-10 rounded-lg px-4" onClick={() => void client.current?.leave()} type="button">Leave</button></main>;
}
